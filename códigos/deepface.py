...
def save_hash(img_dict, model_name = 'Ensemble', model = None, enforce_detection = True):
	...
	if model_name == 'Ensemble':
		...
		df = pd.DataFrame(np.zeros([len(img_list), len(model_names)]),
                  		  columns = model_names, index = list(img_dict.keys()))
...
def ensemble_dataframe(metrics_df):
	...
	comp_pbar = tqdm(range(0, len(metrics_df.index)), desc = 'Realizando o ensemble para cada par de indivíduos!')

	for index in comp_pbar:

		i = metrics_df.index[index]
				
		prediction = deepface_ensemble.predict(np.expand_dims(np.array(metrics_df.loc[i]), axis=0))[0]
		...
		ensembles = [identified, reality, score]

		verification_df.loc[i] = ensembles
...
def f1_calculation(verif_df):
	...
	for i in verif_df.index:

		if (verif_df.loc[i][0] == verif_df.loc[i][1]) and (verif_df.loc[i][0] == 'true'):
			TP += 1
		elif (verif_df.loc[i][0] != verif_df.loc[i][1]) and (verif_df.loc[i][0] == 'true'):
			FP += 1 
		elif (verif_df.loc[i][0] == verif_df.loc[i][1]) and (verif_df.loc[i][0] != 'true'):
			TN += 1
		else:
